...
function indekseGoreFenomen(dizi, index) {
  const secilenFeno = dizi[index];
  return `${index}. indekste bulunan fenomen: ${secilenFeno.profile}`;
}
...
3. Yeni dizinin üzerinde dolaşın ve dizinin her bir indeksinde yalnızca 
fenomenin adına eşit olacak şekilde güncelleyin
...
ÖRNEK: fenomenEkle(fenomenler, 6, "Workintech", 10000000, 2022, "Instagram") çağrıldığında 
dizinin sonuna yukarıdaki nesne en sona eklenerek yeni fenomenler dizisini döndürmelidir. */
...
1. ilk parametre olarak fenomenler dizisini alın
2. Alınan diziden yüzmilyon'dan (100000000) fazla takipçisi olan
 fenomenlerin adlarını içeren bir dizi döndürün.

ÖRNEK: enFenomenler(fenomenler) çağrıldığında sonuç olarak
 ["Instagram", "Cristiano Ronaldo", ... "Khabane lame"] dönemelidir
*/

function enFenomenler(birDizi) {
  const cokFenolar = [];

  for (const item of birDizi) {
    if (item.followers > 100000000) {
      cokFenolar.push(item.profile);
    }
  }
...
function fenomenGonderimSayisi(arrFen, searchProfile) {
  for (let i = 0; i < arrFen.length; i++) {
    if (arrFen[i].profile === searchProfile) return arrFen[i].posts;
  }
}
